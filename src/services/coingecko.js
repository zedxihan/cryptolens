...
export async function getTrendingCoins() {
  try {
    ...
    const data = await fetchJSON(`${BASE_URL}/search/trending`)
    if (!data?.coins) return [];

    const ids = data.coins.map((coin) => coin.item.id).join(",")

    const findCoins = await fetchJSON(
      `${BASE_URL}/coins/markets?vs_currency=usd&ids=${ids}&sparkline=false&price_change_percentage=24h`
    );
    const coinMap = new Map(findCoins.map((coin) => [coin.id, coin]))
